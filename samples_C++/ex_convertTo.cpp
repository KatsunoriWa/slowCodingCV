...
int main(int agrc,  char* argv[]){
	...
	cv::Mat img5 = cv::Mat::zeros(size, CV_32F);
	cv::Mat img6 = cv::Mat::zeros(size, CV_8UC1);
    for(int y =0; y<img1.rows; y++){
        for(int x=0; x< img1.cols; x++){
			img5.at<float>(y,x) = static_cast<float>(img1.at<uchar>(y, x))/255.0;
        }
    }

    for(int y =0; y<img1.rows; y++){
        for(int x=0; x< img1.cols; x++){
			img6.at<uchar>(y,x) = cv::saturate_cast<uchar>(img5.at<float>(y, x)*255);
        }
    }
    
    int64 time3 = cv::getTickCount();
    cv::imwrite("ex_convertTo.png", img3);
    cv::imwrite("ex_convertTo_loop.png", img6);
